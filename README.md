...
**Runs on your own hardware.** No account, no cloud, no telemetry — the panel
fetches straight from a machine you control, on your LAN or wherever you like
to put it. Python 3.10+, nine dependencies, no database, no build step. MIT.
...
```bash
./run.sh                    # makes the venv, installs, serves. Needs Python 3.10+.
docker compose up -d        # or this, if you would rather not have Python about
```

Then open `http://localhost:8080/preview`, and point your display at
`http://<host>:8080/c/main.png`.

**[docs/DEPLOY.md](docs/DEPLOY.md)** covers all of it properly — Docker, a
plain machine, systemd, Proxmox — plus what to do differently if you are
putting it on the internet rather than a LAN, and a troubleshooting list.

## Documentation

- **[docs/DEPLOY.md](docs/DEPLOY.md)** — running it: Docker, a plain machine,
  systemd, Proxmox; LAN versus the internet; troubleshooting
- **[docs/DEVICE.md](docs/DEVICE.md)** — what the Glance Scroll actually does,
  including behaviour found by experiment rather than documentation
...
## What a panel needs from you

This was built for the **Glance Scroll**, and those constraints shaped
everything. If you have a different display, the numbers will differ but the
shape of the problem will not: it asks for an image on a timer and draws
whatever comes back.

| Constraint | Glance Scroll |
|---|---|
...
**The device fetches for itself.** The image is not uploaded to or proxied
through anybody's servers — the panel asks your server directly. A LAN address
works and no tunnel is needed, as long as the two are on the same network.

Some setup apps validate the URL from their own end when you first add it,
which can mean it has to be reachable from outside for that one moment even
though every fetch afterwards is local. [docs/DEPLOY.md](docs/DEPLOY.md) says
what to do about it.

Traffic to the panel is unencrypted, so treat what it displays accordingly.
...
One dependency is not MIT: **`recurring-ical-events` is LGPL-3.0-or-later**.
Using it from Python the way this does is exactly what the LGPL allows, and it
imposes nothing on your use of this project. It is worth knowing if you
redistribute a built image, where the usual LGPL obligation to let someone
swap the library out applies — with pip-installed Python that is satisfied by
the nature of the thing, but you should know it is there rather than find out
later. Everything else is MIT, BSD or Apache-2.0.

The vocabulary decks, bitmap fonts, sprites and weather icons in this repo are
original and MIT with the rest. Team crests are **not** bundled: they are
fetched from ESPN at runtime and cached locally, because they are not ours to
redistribute.
